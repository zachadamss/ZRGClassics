...
| ID | Item | Status | Notes |
|----|------|--------|-------|
| D1 | Logo: SVG, transparent, Gulf palette | ✅ | Inline SVG lockup (`partials/logo.njk`, `logo-mark.njk`): a 964 in Gulf blue with the Turbo whale tail and Cup wheels. Body, windshield, wheels and arches traced from a straight side photo of a 964 Carrera; window lines from a 964 side drawing; whale tail scaled from a 964 Turbo drawing. favicon.svg, favicon-32, apple-touch-icon, og-default.png, the homepage blueprint and the stamp use the same paths. A simpler small-size mark is the fallback if this doesn't read well in the header. |
...
- **2026-09-24**: logo car redrawn, first as an early long-hood 911 (PR #10), then as a 964 with a whale tail, traced from reference photos and a drawing, in the header, footer, favicon, share image, homepage blueprint, and author stamp; orange stripe dropped; blueprint callouts re-pointed (engine callout at the rear).
